...
namespace fullsail_ai { namespace algorithms {
	...
	bool isGreater(SearchNode* const& lhs, SearchNode* const& rhs)
	{
	...
	public:
		...
		std::unordered_map<Tile*, SearchNode*> visited;
		TileMap*  map;
		SearchNode* start;
		SearchNode* end;
		float hWeight = 1.0f;
		...
		DLLEXPORT  void buildGraph(TileMap* tileMap);

		PriorityQueue<SearchNode*> ordered;
