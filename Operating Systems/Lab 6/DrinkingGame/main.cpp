...
struct DrinkerPool 
{
	...
	int DrinkerThreads = 0;
	std::condition_variable gun_var;
	bool GunFired = false;
};
...
bool TryToGetResources(Drinker *currentDrinker)
{
	...
	//    second resource without blocking to avoid deadlocks. Each resource has a mutex
	//    that will determine which thread 'owns' the specified resource. If the mutex
	//    is not locked then that resource is free to be acquire by a drinker. Each
	//    resource also has a lock count and each drinker has its own a try count, both 
	...
	{
		if (currentDrinker->resourcePool->resources[i].type != firstResource->type)
		{
			if (currentDrinker->resourcePool->resources[i].resourceMutex.try_lock() == true)
			{
...
void StartDrinker(Drinker *currentDrinker)
{
	...
	while(true)
	{
		...
		////////////////////////////////////////////////////////////////////////////////////
		currentDrinker->drinkerPool->CountMutex.lock();
		if (currentDrinker->drinkerPool->stopDrinkingFlag)
		{
			currentDrinker->drinkerPool->CountMutex.unlock();
			break;
		}
		currentDrinker->drinkerPool->CountMutex.unlock();
		////////////////////////////////////////////////////////////////////////////////////
		...
		{
			///////////////////////////////////////////////////////////////////////////////////
			// TODO:: Make the access to the stopDrinkingFlag thread safe. You must not wait if
			//   the stopDrinkingFlag has been set. Failure to do so will likely result in a few
			//   threads getting stuck on the wait forever. 
			///////////////////////////////////////////////////////////////////////////////////
			currentDrinker->drinkerPool->CountMutex.lock();
			if(currentDrinker->drinkerPool->stopDrinkingFlag)
			{
...
void DrinkerThreadEntrypoint(Drinker *currentDrinker)
{
	...
	////////////////////////////////////////////////////////////////////////////////////////////////
	std::unique_lock<std::mutex> locked(currentDrinker->drinkerPool->GunMutex);

	currentDrinker->drinkerPool->CountMutex.lock();
	currentDrinker->drinkerPool->DrinkerThreads++;
	currentDrinker->drinkerPool->con_var.notify_all();
	currentDrinker->drinkerPool->CountMutex.unlock();

	currentDrinker->drinkerPool->gun_var.wait(locked, [currentDrinker]() { return currentDrinker->drinkerPool->GunFired == true; });
...
int main(int argc, char **argv)
{
	...
	///////////////////////////////////////////////////////////////////////////////////
	std::unique_lock<std::mutex> locked(poolOfDrinkers.CountMutex);
	for (int i = 0; i < drinkerCount; i++)
	{
	...
	///////////////////////////////////////////////////////////////////////////////////
	poolOfDrinkers.con_var.wait(locked, [&]() { return poolOfDrinkers.DrinkerThreads == drinkerCount; });
	locked.unlock();
	...
	///////////////////////////////////////////////////////////////////////////////////
	poolOfDrinkers.GunMutex.lock();
	poolOfDrinkers.GunFired = true;
	poolOfDrinkers.gun_var.notify_all();
	poolOfDrinkers.GunMutex.unlock();
	...
	///////////////////////////////////////////////////////////////////////////////////
	locked.lock();
	poolOfDrinkers.con_var.wait(locked, [&]() { return poolOfDrinkers.DrinkerThreads == 0; });
	locked.unlock();
