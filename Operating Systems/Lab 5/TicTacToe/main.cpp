...
///////////////////////////////////////////////////////////////////////////////////
// Controls all access to the lockable object used for synchronizing output.
//
// Arguments:
//   operationToperform - Operation we'll be performing on the lockable object.
// 
// Note: 
//   This function will primarily be used in your synchronized print function,
...
void LogSync(LogSyncOperation operationToPerform)
{
	...
	if (operationToPerform == LogSyncOperation::Lock)
		mMutex.lock();
	if (operationToPerform == LogSyncOperation::Unlock)
		mMutex.unlock();
...
//   calling this function. You will need to extract these functions into a
//   va_list to be able to do anything with them. After you have obtained the
//   list of arguments into the va_list then you can use vprintf to print the
//   original formatted string exactly like printf would. Just keep in mind the
//   whole purpose of intercepting the printf: We want to make it so one and only
//   one thread will be able to print at a time. The need for this will be made
...
void PlayerThreadEntrypoint(Player *currentPlayer)
{
	...
	std::unique_lock<std::mutex> locked(currentPlayer->playerPool->GunMutex);


	currentPlayer->playerPool->CountMutex.lock();
	currentPlayer->playerPool->PlayerThreads++;
	currentPlayer->playerPool->con_var.notify_all();
	currentPlayer->playerPool->CountMutex.unlock();



	
	currentPlayer->playerPool->gun_var.wait(locked, [currentPlayer]() { return currentPlayer->playerPool->GunFired == true; });
...
int main(int argc, char **argv)
{
	...
	///////////////////////////////////////////////////////////////////////////////////
	// TODO:: Initialize pool of players
	///////////////////////////////////////////////////////////////////////////////////

	poolOfPlayers.PlayerThreads = 0;
	...
	///////////////////////////////////////////////////////////////////////////////////
	std::unique_lock<std::mutex> locked(poolOfPlayers.CountMutex);
	for (int i = 0; i < totalPlayerCount; i++)
	{
	...
	///////////////////////////////////////////////////////////////////////////////////
	poolOfPlayers.con_var.wait(locked, [&]() { return poolOfPlayers.PlayerThreads == totalPlayerCount; });
	locked.unlock();
	///////////////////////////////////////////////////////////////////////////////////
	...
	poolOfPlayers.gun_var.notify_all();
	poolOfPlayers.GunMutex.unlock();
	///////////////////////////////////////////////////////////////////////////////////
	// TODO:: Wait for all detached player threads to complete.
	///////////////////////////////////////////////////////////////////////////////////
	locked.lock();
	poolOfPlayers.con_var.wait(locked, [&]() { return poolOfPlayers.PlayerThreads == 0; });
	locked.unlock();
